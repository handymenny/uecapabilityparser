...
internal class ImportCapabilityInformationJsonTest {
    ...
    @Test
    fun ueCapAddJson() {
        val capabilities = importerJson.parse(getResourceAsText("/newEngine/input/json/ueCapAdd.json")!!)
        ...
        // LTE Combos
        val expectedCsv = getResourceAsStream("/newEngine/oracle/ueCapAdd.csv")!!.bufferedReader()
            .readLines().dropLastWhile { it.isBlank() }
        val actualCsv = Utility.toCsv(capabilities).lines().dropLastWhile { it.isBlank() }
        assertLinesMatch(expectedCsv, actualCsv)

        // LTE bands
        val expectedLteBands = listOf(
    ...
    fun ueCapReducedJson() {
        val capabilities = importerJson.parse(getResourceAsText("/newEngine/input/json/ueCapReduced.json")!!)
        ...
        // LTE Combos
        val expectedCsv = getResourceAsStream("/newEngine/oracle/ueCapReduced.csv")!!.bufferedReader()
            .readLines().dropLastWhile { it.isBlank() }
        val actualCsv = Utility.toCsv(capabilities).lines().dropLastWhile { it.isBlank() }
        assertLinesMatch(expectedCsv, actualCsv)
    ...
    @Test
    fun ueCap1024qam() {
        val capabilities = importerJson.parse(getResourceAsText("/newEngine/input/json/ueCap1024qam.json")!!)
        ...
        // LTE Combos
        val expectedCsv = getResourceAsStream("/newEngine/oracle/ueCap1024qam.csv")!!.bufferedReader()
            .readLines().dropLastWhile { it.isBlank() }
        val actualCsv = Utility.toCsv(capabilities).lines().dropLastWhile { it.isBlank() }
        assertLinesMatch(expectedCsv, actualCsv)
    ...
    fun ueCapReduced1024qam() {
        val capabilities = importerJson.parse(getResourceAsText("/newEngine/input/json/ueCapReduced1024qam.json")!!)
        ...
        // LTE Combos
        val expectedCsv = getResourceAsStream("/newEngine/oracle/ueCapReduced1024qam.csv")!!.bufferedReader()
            .readLines().dropLastWhile { it.isBlank() }
        val actualCsv = Utility.toCsv(capabilities).lines().dropLastWhile { it.isBlank() }
        assertLinesMatch(expectedCsv, actualCsv)
